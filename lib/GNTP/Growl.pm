...
EOF
    ...
    for my $notification ( @{$notifications} ) {
        $count++;
        my %data = (
            Name => $notification->{Name} || "GNTP::Growl Notify$count",
            DisplayName => $notification->{DisplayName}
              || $notification->{Name} || "GNTP::Growl Notify$count",
...
GNTP::Growl - Perl implementation of GNTP Protocol (Client Part)
...
GNTP::Growl is Perl implementation of GNTP Protocol (Client Part)
...
Initialize GNTP::Growl object. You can set few parameter of
IO::Socket::INET. and application name will be given 'GNTP::Growl' if you
does not specify it.
